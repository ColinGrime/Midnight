package me.colingrimes.plugin.command.display.actionbar;
...
public class DisplayActionBar implements Command<Midnight> {
	...
	public void execute(@Nonnull Midnight plugin, @Nonnull Sender sender, @Nonnull ArgumentList args) {
		...
		Display display = plugin.display().createActionBar(text);
		...
		Messages.ACTIONBAR_SHOW.sendTo(sender, Placeholders.of("{player}", player.get().getName()));
	}

	@Override
	public void configureProperties(@Nonnull CommandProperties properties) {
		properties.setUsage(Messages.ACTIONBAR_USAGE);
